...
import {forceEntityInXML} from 'opensand/api/index.ts';
import {useSelector, useDispatch} from 'opensand/redux/index.ts';
import type {IActions} from 'opensand/utils/actions.ts';
import {useTimer} from 'opensand/utils/hooks.tsx';
...
const XsdParameter: React.FC<XsdProps> = (props) => {
    ...
    const loading = useSelector((state: { model: { status: any; }; }) => state.model.status);
    const templates = useSelector((state: { form: { templates: any; }; }) => state.form.templates);
...
const Parameter: React.FC<Props> = (props) => {
    ...
    const enumeration = model?.environment?.enums?.find((e: { id: any; }) => e.id === parameter.type);
    if (enumeration != null) {
        return (
            <EnumParam
                parameter={parameter}
