...
export class ModelEntity {
    ...
    async createEntity() {
        ...
        const result_inf =  await parseStringPromise(jsonData_inf.content, { explicitArray: false });
        this.infrastructure = result_inf

        const response_pr = await fetch(urlprofileDefault)
        const jsonData_pr =  await response_pr.json();
        const result_pr =  await parseStringPromise(jsonData_pr.content, { explicitArray: false });
