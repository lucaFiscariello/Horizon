...
class NSTBuilder:
    ...
    def __init__(self):
        with open(PATH_TEMPLATE_NST, 'r') as yaml_file:
            ...
            self.json_template_nst = json.loads(json_template_nst)


    def set_id_nst(self,id):
        set_value(self.json_template_nst, ID_NST_PATH, id)

    def set_name_nst(self,name):
        set_value(self.json_template_nst, NAME_NST_PATH, name)
    ...
    def add_nst_subnet(self,subnet_id,desc,nsd_ref,num_subnet):
        ...
        set_value(self.json_template_nst, NST_SUBNET_PATH, copy.deepcopy(json_template_nst_subnet),isElementOfArray=True)
        set_value(self.json_template_nst, NST_SUBNET_ID_PATH,subnet_id)
        set_value(self.json_template_nst, NST_SUBNET_DESC_PATH,desc)
        set_value(self.json_template_nst, NST_SUBNET_NSREF_PATH,nsd_ref)
    ...
    def add_nst_vld(self,vld_id,vld_name,num_vld,mng_net=False):
        ...
        set_value(self.json_template_nst,NST_VLD_PATH, copy.deepcopy(json_template_nst_vld),isElementOfArray=True)
        set_value(self.json_template_nst,NST_VLD_PATH_NAME, vld_id )
        set_value(self.json_template_nst,NST_VLD_PATH_ID, vld_name )

        if mng_net:
            set_value(self.json_template_nst,NST_VLD_PATH_MNG, True )

    def add_nst_nss_cp(self,cp_id,cp_nsd_id,num_vld,num_cp):
        NST_VLD_CP_PATH[NST_VLD_POS] = num_vld
        set_value(self.json_template_nst,NST_VLD_CP_PATH, copy.deepcopy(json_template_nst_cp),isElementOfArray=True)
        
        NST_VLD_CP_NS_PATH[NST_VLD_POS]=num_vld
        NST_VLD_CP_NS_PATH[NST_VLD_CP_POS]=num_cp
        set_value(self.json_template_nst,NST_VLD_CP_NS_PATH, cp_id)

        NST_VLD_CP_NSDCP_PATH[NST_VLD_POS]=num_vld
        NST_VLD_CP_NSDCP_PATH[NST_VLD_CP_POS]=num_cp
        set_value(self.json_template_nst,NST_VLD_CP_NSDCP_PATH, cp_nsd_id)


    def build(self):
        return self.json_template_nst
