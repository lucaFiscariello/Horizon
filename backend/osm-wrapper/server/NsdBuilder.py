...
class NSDBuilder:
    ...
    def set_id_ns(self,id):
        set_value(json_template, ID_PATH,id)

    def set_name_ns(self,name):
        set_value(json_template, NAME_PATH,name)

    def set_versione_ns(self,version):
        set_value(json_template, VERSION_PATH,version)

    def add_vnf_id_inside_ns(self,vnf_id):
        set_value(json_template, VNF_NS_ID_PATH,vnf_id,isElementOfArray=True)

    def add_vld_id_inside_ns(self,vld_id,mng_net=False):
        if mng_net :
            set_value(json_template, VLD_NS_PATH,{"id":vld_id,"mgmt-network":True},isElementOfArray=True)
        else:
            set_value(json_template, VLD_NS_PATH,{"id":vld_id},isElementOfArray=True)
    ...
    def add_sapd(self,id,virtual_link_desc,num_sapd):
        ...
        set_value(json_template, SAPD_PATH_ID, id)
        set_value(json_template, SAPD_PATH_VLD, virtual_link_desc)
    ...
    def add_vlp(self,vlp_id,vlp_vld,cidr,num_vlp):
        ...
        set_value(json_template, VLP_ID_PATH, vlp_id )
        set_value(json_template, VLP_VLD_PATH, vlp_vld )
        set_value(json_template, VLP_VLD_CIDR, cidr )
    ...
    def add_vnf(self,vnf_id,vnf_profile_id,num_vnf):
        ...
        set_value(json_template, VNFD_ID_PATH,vnf_profile_id)
        set_value(json_template, VNF_PROFILE_ID_PATH,vnf_id )
    ...
    def add_vlc_on_vnf(self,cdp_element,cdp_id,vpl_id,num_cpd,num_vnf):
        ...
        set_value(json_template, VNF_CPD_ELEMENT_PATH,cdp_element )
        set_value(json_template, VNF_CPD_ID_PATH,cdp_id )
        set_value(json_template, VNF_VLP_ID_PATH,vpl_id )
